...
void Checker( map<string, Module> &moduleMap )
{
	...
	for ( unsigned int i = 0; i < isoNodes.size(); i++ ) {
		...
		if ( !CheckISO_2_3( isoNodes[i], moduleMap ) )
			continue;

		if ( !CheckISO_5( isoNodes[i], moduleMap ) )
			continue;

		if ( !CheckISO_6( isoNodes[i], moduleMap ) )
			continue;
...
bool CheckISO_2_3( NodePointer ISONodes, map<string, Module> &moduleMap )
{
    ...
    if ( source == destination ) {
		...
		s = "[Instance: " + ISONodes->hiraName + " (" + ISONodes->type + ") Source and destination power domain: " + source + "]";
	...
	if ( locate == "from" ) {
		if ( ISONodes->domain == destination ) {
			...
			s = "['" + ISONodes->hiraName + "' (" + ISONodes->type + ") is not allowed in source domain\n";
		...
		cout << "********  Unknown iso cell location for: " << ISONodes->hiraName << endl;
...
bool CheckLS_1_2_4( NodePointer LSNodes, map<string, Module> &moduleMap )
{
	...
	if ( (LSNodes->pre.size() != 0) && (LSNodes->suc.size() != 0) ) {
		if ( source == destination ) {
			...
			string finalport2;
			string finalsource2 = findS( moduleMap, LSNodes->hiraName, OUT, finalport2 );
			
			s = "[Level shifter '" + LSNodes->hiraName + "' (" + LSNodes->type + ") has the same input/output voltage domain in all power modes\n";
			s += "\t\t\t Input pin driver: " + soutce_name + "/" + finalsource + "/" + finalport + "\n";
			s += "\t\t\t Output pin load: " + destination_name + "/" + finalsource2 + "/" + finalport2 + "\n";
	...
	for( i = 0; i < createPowMode.size(); i++ ) {
		...
		if ( createPowMode[i].defMode == false )
			continue;
		...
		if ( (LSNodes->pre.size() != 0) && chLS1 ) {
			if ( source_vol > 0 ) {
				if( (source_vol < inLow) || (source_vol > inHigh) ) {
					...
					s = "[Pin '" + findPort(LSNodes->hiraName, IN, moduleMap, "-1") + "' of '" + LSNodes->hiraName + "' (module " + LSNodes->type + " ) is connected to domain "
						 + source + " (Power Voltage " + float2str(source_vol) + "V)\n";
		...
		if ( (LSNodes->suc.size() != 0) && chLS2 ) {
			if ( dest_vol > 0 ) {
				if( (dest_vol < outLow) || (dest_vol > outHigh) ) {
					...
					s = "[Pin '" + findPort(LSNodes->hiraName, OUT, moduleMap, "-1") + "' of '" + LSNodes->hiraName + "' (module " + LSNodes->type + " ) is connected to domain "
						 + destination + " (Power Voltage " + float2str(dest_vol) + "V)\n";
					s += "\t\t\t Expected voltage domain is ";
...
{
	string signal = moduleMap[ISONode->father->type].units[ISONode->name].ports[defineIsoCell[ISONode->type].enable].connectWireName;
	...
	for ( size_t i = 0; i < createPowMode.size(); i++ ) {
		 if ( createNomCondition[createPowMode[i].condition[topPortDomain[signal].domain]].voltage == 0 
			&& createNomCondition[createPowMode[i].condition[ISONode->domain]].voltage != 0.0 ) {
			
			string s;
			
			s = "[Pin: '"  + signal + "' in control path can be OFF when the isolation control pin '" + ISONode->hiraName + "/" + defineIsoCell[ISONode->type].enable + "' is ON]";
			
			Report[0].str.push_back( s );
			
			
			//cout << "Error: Pin: \'" << signal << "\' in control path can be OFF when the isolation control pin \'" << ISONode->hiraName << "/" << defineIsoCell[ISONode->type].enable << "\'" << endl;
			break;
		}
	}
...
bool CheckISO_4_LS_3( map<string, Module> &moduleMap )
{
	...
	for ( nodeIter = nodeMap.begin(); nodeIter != nodeMap.end(); nodeIter++  ) {
		...
		for ( i = 0; i < nodeIter->second.suc.size(); i++ ) {
			...
			if ( isSpeciallInstance( sucIter->second.type, sucIter->second.hiraName ) ) // suc instance is special case
				continue;


			bool isIgnore = false;
			for ( k = 0; k < sucIter->second.children.size(); k++ ) {
				if ( isSpeciallInstance(sucIter->second.children[k]->type, sucIter->second.children[k]->name) ) {
					isIgnore = true;
					break;
				}
			}
			...
			// Check LS 3
			if ( createLevShRule.find( nodeIter->second.domain+sucIter->second.domain ) != createLevShRule.end() ) {
				float source_vol = 0;
				float dest_vol = 0;
					
				for( unsigned int k = 0; k < createPowMode.size(); k++ ) {
					if ( createPowMode[k].defMode == false )
						continue;
				
					string source_name = CPM_DomainToNom( createPowMode[k], nodeIter->second.domain );
					string dest_name = CPM_DomainToNom( createPowMode[k], sucIter->second.domain );
				...
				string finalport;
				string finalsource = findS( moduleMap, nodeIter->second.hiraName, OUT, finalport );
				
				s = "[Driver Pin: '" + nodeIter->second.hiraName + "/" + finalsource + "/" + finalport + "'(voltage: " + float2str(source_vol) + "V) to Load Pin: '" + sucIter->second.hiraName + "/" + findPort(sucIter->second.hiraName, IN, moduleMap, "-1") + "'(voltage: " + float2str(dest_vol) + "V) does not have level shifter cell]";
			...
			for( j = 0; j < createPowMode.size(); j++ ) {
				string source_name = CPM_DomainToNom( createPowMode[j], nodeIter->second.domain );
				string dest_name = CPM_DomainToNom( createPowMode[j], sucIter->second.domain );
				float source_vol = CNC_NameToVoltage( source_name );
				float dest_vol = CNC_NameToVoltage( dest_name );

				C_I_R* c = &createIsoRule[nodeIter->second.domain+sucIter->second.domain];
				
				if ( (source_vol > 0) && (dest_vol == 0) ) { // On->off
					string s;
					
					s = "[Pin: '" + nodeIter->second.hiraName + "/" + findPort(nodeIter->second.hiraName, IN, moduleMap, "-1") + "' in rule '" +  c->name + "' does not have isolation cell]";
...
{
	if ( defineIsoCell.find( type ) != defineIsoCell.end() )
		return true;

	if ( defineLevShCell.find( type ) != defineLevShCell.end() )
		return true;
...
bool CheckISO_5( NodePointer ISONodes, map<string, Module> &moduleMap )
{
	...
	if ( moduleMap[father->type].units[ISONodes->name].ports[defineIsoCell[ISONodes->type].enable].connectWireName != 
		createIsoRule[source+destination].condition ) {
		
		string s;
		D_I_C* cell = &defineIsoCell[ISONodes->type];
		C_I_R* c = &createIsoRule[source+destination];

		s = "[The control pin '" + cell->enable + "' of '" + ISONodes->hiraName + "' is connected to a wrong pin. \n";
		s += "\t\t\t Rule '" + createIsoRule[source+destination].name + "' - specified control pin is ";
...
bool CheckISO_6( NodePointer ISONodes, map<string, Module> &moduleMap )
{
	...
	if ( t < 4 ) {
		if ( table[t][A*4+B*2+C*1] != EN ) {
			...
			s = "[The control pin '" + control + "' of '" + ISONodes->hiraName + "' is connected to the correct pin but with wrong polarity\n";
...
string findS( map<string, Module> &moduleMap, string hirname, PortType source_type, string &finalport )
{
	// LS Cell
...
string findD( map<string, Module> &moduleMap, string hirname, PortType source_type, string &finalport )
{
	// LS Cell
	NodePointer nowNode = &nodeMap[hirname];				// LS
	//Module* nowModule = &moduleMap[nowNode->type];		// LS
