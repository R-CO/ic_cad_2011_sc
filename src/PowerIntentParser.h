...
void DIC_Handle(const string &, map<string, Module> &);		// define_isolation_cell handle
...
/*** global variables ***/
map<string, C_P_D> createPowDomain;		// using the power domain name to be the index
map<string, C_I_R> createIsoRule;		// using from + to to be the index
map<string, D_I_C> defineIsoCell;		// using the isolation cell name to be the index
map<string, C_L_S_R> createLevShRule;	// using from + to to be the index
map<string, D_L_S_C> defineLevShCell;	// using the level shifter cell name to be the index
map<string, C_N_C> createNomCondition;	// using the nominal condition name to be the index
...
void ParsePowerFile(const vector<string> &powerStatement, map<string, Module> &moduleMap)
{
     size_t i;
     ...
     for ( i = 0; i != powerStatement.size(); i++ ) {
         if ( powerStatement[i].find("define_isolation_cell") != string::npos ) {
         ...
         else if ( powerStatement[i].find("create_isolation_rule") != string::npos ) {
              CIR_Handle(powerStatement[i]);
         }
         else if ( powerStatement[i].find("create_level_shifter_rule") != string::npos ) {
              CLSR_Handle(powerStatement[i]);
         }
         else if ( powerStatement[i].find("define_level_shifter_cell") != string::npos ) {
              DLSC_Handle(powerStatement[i]);
         }
         else if ( powerStatement[i].find("create_nominal_condition") != string::npos ) {
            ...
            cerr << "Warning: this program can not handle this:" << powerStatement[i] << "***" << endl;
         }
     }
}
...
void DIC_Handle(const string &powerStatement, map<string, Module> &moduleMap)
{
     ...
     while ( true ) {
         if ( tempString == "-cells" ) {
            ...
            else if ( it->second.unitType == "nand" || it->second.unitType == "NAND" ) {
