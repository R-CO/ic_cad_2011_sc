...
string ModuleHandle(const string &, map<string, Module> &); // the return value is the current parsing module name
...
bool ReadVerilogFile(const char *fileName, vector<string> &fileContent, int &fileCount)
{
     string tempString;
     size_t begin, end;
     size_t stringSize;
...
void StoreVerilogStatement(vector<string> &fileContent, vector<string> &veriStatements, const int fileCount)
{
     size_t i;
     ...
     for ( i = 0; i < fileContent[fileCount].length(); i += 2 ) {
         ...
         veriStatements.push_back(fileContent[fileCount].substr(statementBegin, stringSize)); 
         i = statementEnd; 
    } 
}

void ParseVerilogFile(vector<string> &veriStatements, map<string, Module> &moduleMap)
{
...
string ModuleHandle(const string &veriStatement, map<string, Module> &moduleMap)
{
...
void InputHandle(const string &veriStatement, map<string, Module> &moduleMap, const string &currentModule)
{
    ...
    for ( i = 0; i < commaCount; i++ ) {
        ...
        if ( moduleMap[currentModule].ports.count(tempString) != 0 ) {
            ...
            cerr << "There is an port specific error in module \"" << currentModule << "!" << endl;
            return;
        }
        begin = veriStatement.find_first_not_of(" ,", end);
    }        
...
void OutputHandle(const string &veriStatement, map<string, Module> &moduleMap, const string &currentModule)
{
    bool multiBits = false;
    int from = 0, to = 0;
    size_t commaCount = 0;
...
bool InstanceHandle(const string &veriStatement, map<string, Module> &moduleMap, const string &currentModule)
{
            ...
            switch ( veriStatement[i] ) {
                case '.':
                    ...
                    if ( moduleMap[tempUnit.unitType].ports.count(portName) > 0 ) {
                        ...
                        tempUnit.ports[portName] = moduleMap[tempUnit.unitType].ports[portName];
                ...
                case '}':
                    rightBig = i;
                    tempString = veriStatement.substr(leftBig.back(), rightBig-leftBig.back());
                    ...
                    if ( commaCount > 0 ) {
                        ...
                        while ( commaCount > 0 ) {
                            ...
                            if ( wireName.find('[') != string::npos && wireName.find(':') != string::npos ) {
                                GetFromTo(wireName, from, to);
                                tempEnd = wireName.find('[');
                                if ( from == moduleMap[currentModule].wires[wireName.substr(0,tempEnd)].from && 
                                    to == moduleMap[currentModule].wires[wireName.substr(0, tempEnd)].to ) {
                                    ...
                                    tempConInfo.portOrder = portIter->second.portOrder;
                                    moduleMap[currentModule].wires[wireName.substr(0, tempEnd)].conInfo.push_back(tempConInfo);
                                    tempUnit.ports[portName].connectWireName = wireName.substr(0,tempEnd);
                                    tempUnit.ports[portName].inverted = inverted;
                                    tempConInfo.clear();
                                }
                                for ( k = to; k >= from; k--, j-- ) {
                                    tempPortName = portName+'['+IntToString(j)+']';
                                    tempWireName = wireName.substr(0, tempEnd)+'['+IntToString(k)+']';
                                    tempUnit.ports[tempPortName] = moduleMap[tempUnit.unitType].ports[tempPortName];
                                    ...
                                    moduleMap[currentModule].wires[tempWireName].conInfo.push_back(tempConInfo);
                                    tempConInfo.clear();
                                }
                            }
                            else if ( moduleMap[currentModule].wires[wireName].multiBits == true ) {
                                tempConInfo.instanceType = tempUnit.unitType;
                                tempConInfo.instanceName = tempUnit.unitName;
                                tempConInfo.portName = portIter->second.portName;
                                tempConInfo.portType = portIter->second.type;
                ...
                case ')':
                    ...
                    rightSmall = i;
                    tempString = veriStatement.substr(leftSmall.back(), rightSmall-leftSmall.back());
                    leftSmall.pop_back();
...
void WireHandle(const string &veriStatement, map<string, Module> &moduleMap, const string &currentModule)
{
     bool multiBits(false);
     size_t i(0);
     int bit(0);
