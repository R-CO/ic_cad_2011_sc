...
bool ArgumentHandle(const int argc, char *argv[], map<string, Module> &moduleMap)
{
    ...
    for ( size_t i = 0; i != arg["-library"].size(); i++ ) {
        ...
        ParseVerilogFile(veriStatements, moduleMap);
        if ( !ok ) {
            cerr << "This program terminated when parsing the file: " << arg["-library"][i] << endl;
            return false;
        }
        fileCount++;
    }
    ...
    if ( !ReadPowerFile(arg["-power_intent"].back().c_str(), powerStatements) ) {
       cerr << "Can not open the file(" << arg["-power_intent"].back() << ")." << endl;
       return false;
    }
	...
	fout.open( arg["-report"].back().c_str() );
	if ( !fout ) {
...
void CreateGraph(map<string, Module> &moduleMap)
{
    ...
    for ( nodeIter = nodeMap.begin(); nodeIter != nodeMap.end(); ++nodeIter ) {
        ...
        for ( portIter = moduleMap[nodeIter->second.father->type].units[nodeIter->second.name].ports.begin();
            ...
            if ( portIter->second.type == IN ) {
                  for ( i = 0; i < moduleMap[nodeIter->second.father->type].wires[portIter->second.connectWireName].conInfo.size(); i++ ) {
                      tempCon = &moduleMap[nodeIter->second.father->type].wires[portIter->second.connectWireName].conInfo[i];
                      if ( tempCon->instanceType == nodeIter->second.father->type ) {
                         if ( nodeIter->second.father->type == topModule ) {
                            ...
                            if ( pre.count(nodeIter->second.father->hiraName) == 0 ) {
                                ...
                                nodeIter->second.pre.push_back(nodeIter->second.father->hiraName);
                            }
                         }
                         continue;
                      }
                      if ( moduleMap[tempCon->instanceType].ports[tempCon->portName].type == OUT ) {
                         if ( nodeIter->second.father->type == topModule ) {
                            ...
                            if ( pre.count(nodeIter->second.father->hiraName+'/'+tempCon->instanceName) == 0 ) {
                                pre.insert(nodeIter->second.father->hiraName+'/'+tempCon->instanceName);
                                nodeIter->second.pre.push_back(nodeIter->second.father->hiraName+'/'+tempCon->instanceName);
                            }
                         }
                      }
                  }
                      ...
                      if ( tempCon->instanceType == nodeIter->second.father->type ) {
                          if ( nodeIter->second.father->type == topModule ) {
                                ...
                                if ( suc.count(nodeIter->second.father->hiraName) == 0 ) {
                                    ...
                                    nodeIter->second.suc.push_back(nodeIter->second.father->hiraName);
                                }
                          }
                          continue;
                      }
                      if ( moduleMap[tempCon->instanceType].ports[tempCon->portName].type == IN ) {
                         if ( nodeIter->second.father->type == topModule ) {
                            ...
                            if ( suc.count(nodeIter->second.father->hiraName+'/'+tempCon->instanceName) == 0 ) {
                                suc.insert(nodeIter->second.father->hiraName+'/'+tempCon->instanceName);
                                nodeIter->second.suc.push_back(nodeIter->second.father->hiraName+'/'+tempCon->instanceName);
                            }
                         }
                      }
                  }
		...
		for ( iPort = iUnit->second.ports.begin(); iPort != iUnit->second.ports.end(); ++iPort ) {
			...
			for ( iWireCon = iWire->second.conInfo.begin(); iWireCon != iWire->second.conInfo.end(); ++iWireCon ) {
				if ( iWireCon->instanceType == nodeIter->second.father->type ) {
					...
					tPortInfo.conNode = &nodeMap[nodeIter->second.father->hiraName+'/'+iWireCon->instanceName];
				}
				else {
				...
				if ( iPort->second.type == IN ) {
					if ( (tPortInfo.conPortType == OUT) || (tPortInfo.conNode == nodeIter->second.father &&
						tPortInfo.conPortType == IN) ) {
						tempNodeDomain.node = tPortInfo.conNode->hiraName;
						if ( tPortInfo.conNode->type == topModule ) {
							tempNodeDomain.domain = topPortDomain[tPortInfo.conPortName].domain;
						}
						else {
				...
				else if ( iPort->second.type == OUT ) {
					if ( (tPortInfo.conPortType == IN) || (tPortInfo.conNode == nodeIter->second.father &&
						tPortInfo.conPortType == OUT) ) {
						tempNodeDomain.node = tPortInfo.conNode->hiraName;
						if ( tPortInfo.conNode->type == topModule ) {
							tempNodeDomain.domain = topPortDomain[tPortInfo.conPortName].domain;
						}
